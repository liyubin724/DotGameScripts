    ...
    public class RegisterObjectData
    {
        ...
        void RegisterLuaObject(LuaTable objTable)
        {
            for (int i = 0; i < objectDatas.Length; i++)
            {
                ...
                {
                    LogUtil.LogError(typeof(RegisterObjectData), "LuaBehaviour::RegisterLuaObjects->obj or regObj is Null");
                    continue;
                }
        ...
        void RegisterLuaObjectArr(LuaEnv luaEnv, LuaTable objTable)
        {
            if (objectArrayDatas != null && objectArrayDatas.Length > 0)
            {
                for (int i = 0; i < objectArrayDatas.Length; i++)
                {
                    ...
                    {
                        LogUtil.LogError(typeof(RegisterObjectData), "LuaBehaviour::RegisterLuaObjectArr->Group Name is Null, index = " + i);
                        continue;
                    }
                    ...
                    if (luaObjs != null && luaObjs.Length > 0)
                    {
                        for (int j = 0; j < luaObjs.Length; j++)
                        {
                            ...
                            {
                                LogUtil.LogError(typeof(RegisterObjectData), "LuaBehaviour::RegisterLuaObjectArr->obj or regObj is Null");
                                continue;
                            }
